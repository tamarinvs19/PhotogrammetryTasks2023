...
TEST (CeresSolver, HelloWorld1) {
    ...
    // f'(x) = (10-x)' = -1 всегда, а минимум f(x)^2 достигается в x = 5
...
//    return; // TODO 2 удалите эту строку, затем
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // TODO 3: раскомментируйте^, почему он находит не то что ожидалось? Fixed: line 136
...
//        return; // TODO 6 удалите этот return сразу после выполнения TODO 5
    ...
    double coef = line_params[2] != 0 ? line_params[2] : (line_params[1] != 0 ? line_params[1] : (line_params[0] != 0 ? line_params[0] : 1));
    double coef_ideal = ideal_line[2] != 0 ? ideal_line[2] : (ideal_line[1] != 0 ? ideal_line[1] : (ideal_line[0] != 0 ? ideal_line[0] : 1));
    for (int d = 0; d < 3; ++d) {
        ASSERT_NEAR(line_params[d] / coef, ideal_line[d] / coef_ideal, threshold);
    ...
    // через выбросы прямая не обязана проходить
    ...
    // падало на FitLineNoiseAndOutliers и FitLineNoiseAndOutliersWithHuberLoss
    // расстояние сравнивалось без модуля, поэтому не инлайеры с отрицательным расстоянием становились инлайрами
    ...
    if (outliers_fraction == 0 || use_huber) {
        ...
        // проблема аналогична TODO 8, падало на FitLineNoiseAndOutliersWithHuberLoss
