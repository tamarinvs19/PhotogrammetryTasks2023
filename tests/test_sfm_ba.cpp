...
#define NIMGS_LIMIT                           5 // сколько фотографий обрабатывать (можно выставить меньше чтобы ускорить экспериментирование, или в случае если весь датасет не выравнивается)
...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        for (int i = 0; i < 2; i++) {
            point_rotated[i] /= point_rotated[2];
        }
        ...
        T r = point_rotated[0] * point_rotated[0] + point_rotated[1] * point_rotated[1];
        for (int i = 0; i < 2; i++) {
            point_rotated[i] += k1 * r + k2 * r * r;
        }
#endif
        ...
        residuals[0] = point_rotated[0] - observed_x;
        residuals[1] = point_rotated[1] - observed_y;
